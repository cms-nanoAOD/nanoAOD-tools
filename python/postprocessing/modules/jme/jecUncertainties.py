...
class jecUncertProducer(Module):
    def __init__(
        ...
        self.unc_factorized_path = "%s/%s_UncertaintySources_%s.txt" % (
            "%s/src/PhysicsTools/NanoAODTools/data/jme/" %
            os.environ['CMSSW_BASE'], globalTag, jetFlavour)
        ...
        if "/jecUncertProducerCppWorker_cc.so" not in ROOT.gSystem.GetLibraries(
            ...
            if base:
                ROOT.gROOT.ProcessLine(
                    ".L %s/src/jecUncertProducerCppWorker.cc+O" % base)
            else:
                base = "%s/src/PhysicsTools/NanoAODTools" % os.getenv(
                    "CMSSW_BASE")
                ROOT.gSystem.Load("libPhysicsToolsNanoAODTools.so")
                ROOT.gROOT.ProcessLine(
                    ".L %s/interface/jecUncertProducerCppWorker.h" % base)

    def beginJob(self):
        self.vec_uncerts = ROOT.std.vector(str)()
        for x in self.uncerts:
            self.vec_uncerts.push_back(x[0])
        self.worker = ROOT.jecUncertProducerCppWorker(self.unc_factorized_path,
                                                      self.vec_uncerts)

    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        if not self.doCppOutput:
            super(jecUncertProducerCpp,
                  self).beginFile(inputFile, outputFile, inputTree,
                                  wrappedOutputTree)
        else:
...
# python looper
# you can re-use the uncertainty values in modules running after this one
# in the same event loop
jecUncert = lambda: jecUncertProducer("Summer16_07Aug2017_V11_MC")
jecUncertAll = lambda: jecUncertProducer("Summer16_07Aug2017_V11_MC",
                                         allUncerts)
...
jecUncertAll_cpp = lambda: jecUncertProducerCpp("Summer16_07Aug2017_V11_MC",
                                                allUncerts)

# python looper with C++ helper also writing the output, fastest
# you cannot re-use the uncertainty values in modules running after this one
# in the same event loop
jecUncert_cppOut = lambda: jecUncertProducerCpp("Summer16_07Aug2017_V11_MC",
                                                doCppOutput=True)
jecUncertAll_cppOut = lambda: jecUncertProducerCpp(
    "Summer16_07Aug2017_V11_MC", allUncerts, doCppOutput=True)

jecUncertAK4Puppi = lambda: jecUncertProducer("Summer16_07Aug2017_V11_MC",
                                              jetFlavour="AK4PFPuppi")
jecUncertAK8Puppi = lambda: jecUncertProducer(
    "Summer16_07Aug2017_V11_MC", jetFlavour="AK8PFPuppi", jetColl="FatJet")
