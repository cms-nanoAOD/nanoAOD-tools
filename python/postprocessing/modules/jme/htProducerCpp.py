...
# HT producer, unclean jets only (no lepton overlap cleaning, no jet selection)
class htProducerCpp(Module):
    def __init__(self):
        if "/htProducerCppWorker_cc.so" not in ROOT.gSystem.GetLibraries():
            ...
            if base:
                ROOT.gROOT.ProcessLine(".L %s/src/htProducerCppWorker.cc+O" %
                                       base)
            else:
                ...
                ROOT.gROOT.ProcessLine(
                    ".L %s/interface/htProducerCppWorker.h" % base)
