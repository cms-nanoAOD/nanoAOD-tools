...
class jetmetUncertaintiesProducer(Module):
    ...
    def analyze(self, event):
        ...
        for iJet, jet in enumerate(itertools.chain(jets, lowPtJets)):
            ...
            jet_pt_noMuL1L2L3  = self.jetReCalibrator.correct(jet,rho)[0]   if self.jetReCalibrator.correct(jet,rho)[0] > self.unclEnThreshold else jet.pt # only correct the non-mu fraction of the jet if it's above 15 GeV, otherwise take the raw pt
            jet_pt_noMuL1      = self.jetReCalibratorL1.correct(jet,rho)[0] if self.jetReCalibrator.correct(jet,rho)[0] > self.unclEnThreshold else jet.pt # only correct the non-mu fraction of the jet if it's above 15 GeV, otherwise take the raw pt

            if self.jetReCalibratorProd:
              jet_pt_noMuProdL1L2L3   = self.jetReCalibratorProd.correct(jet,rho)[0]   if self.jetReCalibratorProd.correct(jet,rho)[0] > self.unclEnThreshold else jet.pt
              jet_pt_noMuProdL1       = self.jetReCalibratorProdL1.correct(jet,rho)[0] if self.jetReCalibratorProd.correct(jet,rho)[0] > self.unclEnThreshold else jet.pt # only correct the non-mu fraction of the jet if it's above 15 GeV, otherwise take the raw pt
            else:
            ...
            jet_pt_jer      = jet_pt_nom * jet_pt_jerNomVal
            ...
            if self.corrMET and jet_pt_L1L2L3 > self.unclEnThreshold and (jet.neEmEF+jet.chEmEF) < 0.9:
                if not ( self.metBranchName == 'METFixEE2017' and 2.65<abs(jet.eta)<3.14 and jet.pt*(1-jet.rawFactor)<50 ): # do not re-correct for jets that aren't included in METv2 recipe
                    ...
                    met_px_jer     = met_px_jer     - (jet_pt_jer     - jet_pt_L1)*jet_cosPhi 
                    met_py_jer     = met_py_jer     - (jet_pt_jer     - jet_pt_L1)*jet_sinPhi 
                    met_px_jerUp   = met_px_jerUp   - (jet_pt_jerUp   - jet_pt_L1)*jet_cosPhi 
                    met_py_jerUp   = met_py_jerUp   - (jet_pt_jerUp   - jet_pt_L1)*jet_sinPhi 
                    met_px_jerDown = met_px_jerDown - (jet_pt_jerDown - jet_pt_L1)*jet_cosPhi 
                    met_py_jerDown = met_py_jerDown - (jet_pt_jerDown - jet_pt_L1)*jet_sinPhi 
                    for jesUncertainty in self.jesUncertainties:
