...
class jetmetUncertaintiesProducer(Module):
    ...
    def beginFile(self, inputFile, outputFile, inputTree, wrappedOutputTree):
        ...
        self.out.branch("%s_pt_nom" % self.PuppimetBranchName, "F")
        self.out.branch("%s_phi_nom" % self.PuppimetBranchName, "F")
        self.out.branch("%s_pt_nom" % self.RawmetBranchName, "F")
        self.out.branch("%s_phi_nom" % self.RawmetBranchName, "F")
        if not self.isData:
          self.out.branch("%s_pt_jer" % self.metBranchName, "F")
          self.out.branch("%s_phi_jer" % self.metBranchName, "F")
          self.out.branch("%s_pt_jer" % self.PuppimetBranchName, "F")
          self.out.branch("%s_phi_jer" % self.PuppimetBranchName, "F")
          self.out.branch("%s_pt_jer" % self.RawmetBranchName, "F")
          self.out.branch("%s_phi_jer" % self.RawmetBranchName, "F")
          ...
          for shift in [ "Up", "Down" ]:
              ...
              self.out.branch("%s_pt_jer%s" % (self.PuppimetBranchName, shift), "F")
              self.out.branch("%s_phi_jer%s" % (self.PuppimetBranchName, shift), "F")
              self.out.branch("%s_pt_jer%s" % (self.RawmetBranchName, shift), "F")
              self.out.branch("%s_phi_jer%s" % (self.RawmetBranchName, shift), "F")
              for jesUncertainty in self.jesUncertainties:
                  ...
                  self.out.branch("%s_pt_jes%s%s" % (self.PuppimetBranchName, jesUncertainty, shift), "F")
                  self.out.branch("%s_phi_jes%s%s" % (self.PuppimetBranchName, jesUncertainty, shift), "F")
                  self.out.branch("%s_pt_jes%s%s" % (self.RawmetBranchName, jesUncertainty, shift), "F")
                  self.out.branch("%s_phi_jes%s%s" % (self.RawmetBranchName, jesUncertainty, shift), "F")
              self.out.branch("%s_pt_unclustEn%s" % (self.metBranchName, shift), "F")
              self.out.branch("%s_phi_unclustEn%s" % (self.metBranchName, shift), "F")
              self.out.branch("%s_pt_unclustEn%s" % (self.PuppimetBranchName, shift), "F")
              self.out.branch("%s_phi_unclustEn%s" % (self.PuppimetBranchName, shift), "F")
              self.out.branch("%s_pt_unclustEn%s" % (self.RawmetBranchName, shift), "F")
              self.out.branch("%s_phi_unclustEn%s" % (self.RawmetBranchName, shift), "F")
    ...
    def analyze(self, event):
        ...
        ( t1Puppimet_px,       t1Puppimet_py       ) = ( Puppimet.pt*math.cos(Puppimet.phi), Puppimet.pt*math.sin(Puppimet.phi) )
        ( def_Puppimet_px,     def_Puppimet_py     ) = ( Puppimet.pt*math.cos(Puppimet.phi),  Puppimet.pt*math.sin(Puppimet.phi) )
        ( Puppimet_px,         Puppimet_py         ) = ( rawmet.pt*math.cos(rawmet.phi), rawmet.pt*math.sin(rawmet.phi) )
        ( Puppimet_px_nom,     Puppimet_py_nom     ) = ( Puppimet_px, Puppimet_py )
        ( Puppimet_px_jer,     Puppimet_py_jer     ) = ( Puppimet_px, Puppimet_py )
        ( Puppimet_px_jerUp,   Puppimet_py_jerUp   ) = ( Puppimet_px, Puppimet_py )
        ( Puppimet_px_jerDown, Puppimet_py_jerDown ) = ( Puppimet_px, Puppimet_py )
        ( Puppimet_px_jesUp,   Puppimet_py_jesUp   ) = ( {}, {} )
        ( Puppimet_px_jesDown, Puppimet_py_jesDown ) = ( {}, {} )

        for jesUncertainty in self.jesUncertainties:
            Puppimet_px_jesUp[jesUncertainty]   = Puppimet_px
            Puppimet_py_jesUp[jesUncertainty]   = Puppimet_py
            Puppimet_px_jesDown[jesUncertainty] = Puppimet_px
            Puppimet_py_jesDown[jesUncertainty] = Puppimet_py

        ( t1Rawmet_px,       t1Rawmet_py       ) = ( Rawmet.pt*math.cos(Rawmet.phi), Rawmet.pt*math.sin(Rawmet.phi) )
        ( def_Rawmet_px,     def_Rawmet_py     ) = ( Rawmet.pt*math.cos(Rawmet.phi),  Rawmet.pt*math.sin(Rawmet.phi) )
        ( Rawmet_px,         Rawmet_py         ) = ( rawmet.pt*math.cos(rawmet.phi), rawmet.pt*math.sin(rawmet.phi) )
        ( Rawmet_px_nom,     Rawmet_py_nom     ) = ( Rawmet_px, Rawmet_py )
        ( Rawmet_px_jer,     Rawmet_py_jer     ) = ( Rawmet_px, Rawmet_py )
        ( Rawmet_px_jerUp,   Rawmet_py_jerUp   ) = ( Rawmet_px, Rawmet_py )
        ( Rawmet_px_jerDown, Rawmet_py_jerDown ) = ( Rawmet_px, Rawmet_py )
        ( Rawmet_px_jesUp,   Rawmet_py_jesUp   ) = ( {}, {} )
        ( Rawmet_px_jesDown, Rawmet_py_jesDown ) = ( {}, {} )

        for jesUncertainty in self.jesUncertainties:
            Rawmet_px_jesUp[jesUncertainty]   = Rawmet_px
            Rawmet_py_jesUp[jesUncertainty]   = Rawmet_py
            Rawmet_px_jesDown[jesUncertainty] = Rawmet_px
            Rawmet_py_jesDown[jesUncertainty] = Rawmet_py
        ...
        for iJet, jet in enumerate(itertools.chain(jets, lowPtJets)):
            ...
            if jet_pt_L1L2L3 > self.unclEnThreshold and (jet.neEmEF+jet.chEmEF) < 0.9:
                if not ( self.metBranchName == 'METFixEE2017' and 2.65<abs(jet.eta)<3.14 and jet.pt*(1-jet.rawFactor)<50 ): # do not re-correct for jets that aren't included in METv2 recipe
                    ...
                    Puppimet_px_nom     = Puppimet_px_nom     - (jet_pt_L1L2L3  - jet_pt_L1)*jet_cosPhi
                    Puppimet_py_nom     = Puppimet_py_nom     - (jet_pt_L1L2L3  - jet_pt_L1)*jet_sinPhi
                    if not self.isData:
                      Puppimet_px_jer     = Puppimet_px_jer     - (jet_pt_L1L2L3*jet_pt_jerNomVal   - jet_pt_L1)*jet_cosPhi
                      Puppimet_py_jer     = Puppimet_py_jer     - (jet_pt_L1L2L3*jet_pt_jerNomVal   - jet_pt_L1)*jet_sinPhi
                      Puppimet_px_jerUp   = Puppimet_px_jerUp   - (jet_pt_L1L2L3*jet_pt_jerUpVal    - jet_pt_L1)*jet_cosPhi
                      Puppimet_py_jerUp   = Puppimet_py_jerUp   - (jet_pt_L1L2L3*jet_pt_jerUpVal    - jet_pt_L1)*jet_sinPhi
                      Puppimet_px_jerDown = Puppimet_px_jerDown - (jet_pt_L1L2L3*jet_pt_jerDownVal  - jet_pt_L1)*jet_cosPhi
                      Puppimet_py_jerDown = Puppimet_py_jerDown - (jet_pt_L1L2L3*jet_pt_jerDownVal  - jet_pt_L1)*jet_sinPhi
                      for jesUncertainty in self.jesUncertainties:
                          Puppimet_px_jesUp[jesUncertainty]   = Puppimet_px_jesUp[jesUncertainty]   - (jet_pt_jesUpT1[jesUncertainty]   - jet_pt_L1)*jet_cosPhi
                          Puppimet_py_jesUp[jesUncertainty]   = Puppimet_py_jesUp[jesUncertainty]   - (jet_pt_jesUpT1[jesUncertainty]   - jet_pt_L1)*jet_sinPhi
                          Puppimet_px_jesDown[jesUncertainty] = Puppimet_px_jesDown[jesUncertainty] - (jet_pt_jesDownT1[jesUncertainty] - jet_pt_L1)*jet_cosPhi
                          Puppimet_py_jesDown[jesUncertainty] = Puppimet_py_jesDown[jesUncertainty] - (jet_pt_jesDownT1[jesUncertainty] - jet_pt_L1)*jet_sinPhi
                    Rawmet_px_nom     = Rawmet_px_nom     - (jet_pt_L1L2L3  - jet_pt_L1)*jet_cosPhi
                    Rawmet_py_nom     = Rawmet_py_nom     - (jet_pt_L1L2L3  - jet_pt_L1)*jet_sinPhi
                    if not self.isData:
                      Rawmet_px_jer     = Rawmet_px_jer     - (jet_pt_L1L2L3*jet_pt_jerNomVal   - jet_pt_L1)*jet_cosPhi
                      Rawmet_py_jer     = Rawmet_py_jer     - (jet_pt_L1L2L3*jet_pt_jerNomVal   - jet_pt_L1)*jet_sinPhi
                      Rawmet_px_jerUp   = Rawmet_px_jerUp   - (jet_pt_L1L2L3*jet_pt_jerUpVal    - jet_pt_L1)*jet_cosPhi
                      Rawmet_py_jerUp   = Rawmet_py_jerUp   - (jet_pt_L1L2L3*jet_pt_jerUpVal    - jet_pt_L1)*jet_sinPhi
                      Rawmet_px_jerDown = Rawmet_px_jerDown - (jet_pt_L1L2L3*jet_pt_jerDownVal  - jet_pt_L1)*jet_cosPhi
                      Rawmet_py_jerDown = Rawmet_py_jerDown - (jet_pt_L1L2L3*jet_pt_jerDownVal  - jet_pt_L1)*jet_sinPhi
                      for jesUncertainty in self.jesUncertainties:
                          Rawmet_px_jesUp[jesUncertainty]   = Rawmet_px_jesUp[jesUncertainty]   - (jet_pt_jesUpT1[jesUncertainty]   - jet_pt_L1)*jet_cosPhi
                          Rawmet_py_jesUp[jesUncertainty]   = Rawmet_py_jesUp[jesUncertainty]   - (jet_pt_jesUpT1[jesUncertainty]   - jet_pt_L1)*jet_sinPhi
                          Rawmet_px_jesDown[jesUncertainty] = Rawmet_px_jesDown[jesUncertainty] - (jet_pt_jesDownT1[jesUncertainty] - jet_pt_L1)*jet_cosPhi
                          Rawmet_py_jesDown[jesUncertainty] = Rawmet_py_jesDown[jesUncertainty] - (jet_pt_jesDownT1[jesUncertainty] - jet_pt_L1)*jet_sinPhi
        ...
        if self.metBranchName == 'METFixEE2017':
            ...
            def_Puppimet_px += delta_x_T1Jet
            def_Puppimet_py += delta_y_T1Jet 
            ...
            Puppimet_unclEE_x = def_Puppimet_px - t1Puppimet_px
            Puppimet_unclEE_y = def_Puppimet_py - t1Puppimet_py
            Rawmet_unclEE_x = def_Rawmet_px - t1Rawmet_px
            Rawmet_unclEE_y = def_Rawmet_py - t1Rawmet_py
            ...
            Puppimet_px_nom += delta_x_rawJet - Puppimet_unclEE_x
            Puppimet_py_nom += delta_y_rawJet - Puppimet_unclEE_y
            Rawmet_px_nom += delta_x_rawJet - Rawmet_unclEE_x
            Rawmet_py_nom += delta_y_rawJet - Rawmet_unclEE_y
            ...
            if not self.isData:
              ...
              Puppimet_px_jerUp += delta_x_rawJet - Puppimet_unclEE_x
              Puppimet_py_jerUp += delta_y_rawJet - Puppimet_unclEE_y
              Puppimet_px_jerDown += delta_x_rawJet - Puppimet_unclEE_x
              Puppimet_py_jerDown += delta_y_rawJet - Puppimet_unclEE_y
              for jesUncertainty in self.jesUncertainties:
                  Puppimet_px_jesUp[jesUncertainty] += delta_x_rawJet - Puppimet_unclEE_x
                  Puppimet_py_jesUp[jesUncertainty] += delta_y_rawJet - Puppimet_unclEE_y
                  Puppimet_px_jesDown[jesUncertainty] += delta_x_rawJet - Puppimet_unclEE_x
                  Puppimet_py_jesDown[jesUncertainty] += delta_y_rawJet - Puppimet_unclEE_y
              Rawmet_px_jerUp += delta_x_rawJet - Rawmet_unclEE_x
              Rawmet_py_jerUp += delta_y_rawJet - Rawmet_unclEE_y
              Rawmet_px_jerDown += delta_x_rawJet - Rawmet_unclEE_x
              Rawmet_py_jerDown += delta_y_rawJet - Rawmet_unclEE_y
              for jesUncertainty in self.jesUncertainties:
                  Rawmet_px_jesUp[jesUncertainty] += delta_x_rawJet - Rawmet_unclEE_x
                  Rawmet_py_jesUp[jesUncertainty] += delta_y_rawJet - Rawmet_unclEE_y
                  Rawmet_px_jesDown[jesUncertainty] += delta_x_rawJet - Rawmet_unclEE_x
                  Rawmet_py_jesDown[jesUncertainty] += delta_y_rawJet - Rawmet_unclEE_y
        if not self.isData:
          ...
          ( Puppimet_px_unclEnUp,   Puppimet_py_unclEnUp   ) = ( Puppimet_px, Puppimet_py )
          ( Puppimet_px_unclEnDown, Puppimet_py_unclEnDown ) = ( Puppimet_px, Puppimet_py )
          Puppimet_deltaPx_unclEn = getattr(event, self.metBranchName + "_MetUnclustEnUpDeltaX")
          Puppimet_deltaPy_unclEn = getattr(event, self.metBranchName + "_MetUnclustEnUpDeltaY")
          Puppimet_px_unclEnUp    = Puppimet_px_unclEnUp   + Puppimet_deltaPx_unclEn
          Puppimet_py_unclEnUp    = Puppimet_py_unclEnUp   + Puppimet_deltaPy_unclEn
          Puppimet_px_unclEnDown  = Puppimet_px_unclEnDown - Puppimet_deltaPx_unclEn
          Puppimet_py_unclEnDown  = Puppimet_py_unclEnDown - Puppimet_deltaPy_unclEn

          ( Rawmet_px_unclEnUp,   Rawmet_py_unclEnUp   ) = ( Rawmet_px, Rawmet_py )
          ( Rawmet_px_unclEnDown, Rawmet_py_unclEnDown ) = ( Rawmet_px, Rawmet_py )
          Rawmet_deltaPx_unclEn = getattr(event, self.metBranchName + "_MetUnclustEnUpDeltaX")
          Rawmet_deltaPy_unclEn = getattr(event, self.metBranchName + "_MetUnclustEnUpDeltaY")
          Rawmet_px_unclEnUp    = Rawmet_px_unclEnUp   + Rawmet_deltaPx_unclEn
          Rawmet_py_unclEnUp    = Rawmet_py_unclEnUp   + Rawmet_deltaPy_unclEn
          Rawmet_px_unclEnDown  = Rawmet_px_unclEnDown - Rawmet_deltaPx_unclEn
          Rawmet_py_unclEnDown  = Rawmet_py_unclEnDown - Rawmet_deltaPy_unclEn
        ...
        self.out.fillBranch("%s_pt_nom" % self.PuppimetBranchName, math.sqrt(Puppimet_px_nom**2 + Puppimet_py_nom**2))
        self.out.fillBranch("%s_phi_nom" % self.PuppimetBranchName, math.atan2(Puppimet_py_nom, Puppimet_px_nom))
        self.out.fillBranch("%s_pt_nom" % self.RawmetBranchName, math.sqrt(Rawmet_px_nom**2 + Rawmet_py_nom**2))
        self.out.fillBranch("%s_phi_nom" % self.RawmetBranchName, math.atan2(Rawmet_py_nom, Rawmet_px_nom))
        ...
        if not self.isData:
          ...
          self.out.fillBranch("%s_pt_jer" % self.PuppimetBranchName, math.sqrt(Puppimet_px_jer**2 + Puppimet_py_jer**2))
          self.out.fillBranch("%s_phi_jer" % self.PuppimetBranchName, math.atan2(Puppimet_py_jer, Puppimet_px_jer))
          self.out.fillBranch("%s_pt_jerUp" % self.PuppimetBranchName, math.sqrt(Puppimet_px_jerUp**2 + Puppimet_py_jerUp**2))
          self.out.fillBranch("%s_phi_jerUp" % self.PuppimetBranchName, math.atan2(Puppimet_py_jerUp, Puppimet_px_jerUp))
          self.out.fillBranch("%s_pt_jerDown" % self.PuppimetBranchName, math.sqrt(Puppimet_px_jerDown**2 + Puppimet_py_jerDown**2))
          self.out.fillBranch("%s_phi_jerDown" % self.PuppimetBranchName, math.atan2(Puppimet_py_jerDown, Puppimet_px_jerDown))
          
          self.out.fillBranch("%s_pt_jer" % self.RawmetBranchName, math.sqrt(Rawmet_px_jer**2 + Rawmet_py_jer**2))
          self.out.fillBranch("%s_phi_jer" % self.RawmetBranchName, math.atan2(Rawmet_py_jer, Rawmet_px_jer))
          self.out.fillBranch("%s_pt_jerUp" % self.RawmetBranchName, math.sqrt(Rawmet_px_jerUp**2 + Rawmet_py_jerUp**2))
          self.out.fillBranch("%s_phi_jerUp" % self.RawmetBranchName, math.atan2(Rawmet_py_jerUp, Rawmet_px_jerUp))
          self.out.fillBranch("%s_pt_jerDown" % self.RawmetBranchName, math.sqrt(Rawmet_px_jerDown**2 + Rawmet_py_jerDown**2))
          self.out.fillBranch("%s_phi_jerDown" % self.RawmetBranchName, math.atan2(Rawmet_py_jerDown, Rawmet_px_jerDown))
          ...
          for jesUncertainty in self.jesUncertainties:
              ...
              self.out.fillBranch("%s_pt_jes%sUp" % (self.PuppimetBranchName, jesUncertainty), math.sqrt(Puppimet_px_jesUp[jesUncertainty]**2 + Puppimet_py_jesUp[jesUncertainty]**2))
              self.out.fillBranch("%s_phi_jes%sUp" % (self.PuppimetBranchName, jesUncertainty), math.atan2(Puppimet_py_jesUp[jesUncertainty], Puppimet_px_jesUp[jesUncertainty]))
              self.out.fillBranch("%s_pt_jes%sDown" % (self.PuppimetBranchName, jesUncertainty), math.sqrt(Puppimet_px_jesDown[jesUncertainty]**2 + Puppimet_py_jesDown[jesUncertainty]**2))
              self.out.fillBranch("%s_phi_jes%sDown" % (self.PuppimetBranchName, jesUncertainty), math.atan2(Puppimet_py_jesDown[jesUncertainty], Puppimet_px_jesDown[jesUncertainty]))

              self.out.fillBranch("%s_pt_jes%sUp" % (self.RawmetBranchName, jesUncertainty), math.sqrt(Rawmet_px_jesUp[jesUncertainty]**2 + Rawmet_py_jesUp[jesUncertainty]**2))
              self.out.fillBranch("%s_phi_jes%sUp" % (self.RawmetBranchName, jesUncertainty), math.atan2(Rawmet_py_jesUp[jesUncertainty], Rawmet_px_jesUp[jesUncertainty]))
              self.out.fillBranch("%s_pt_jes%sDown" % (self.RawmetBranchName, jesUncertainty), math.sqrt(Rawmet_px_jesDown[jesUncertainty]**2 + Rawmet_py_jesDown[jesUncertainty]**2))
              self.out.fillBranch("%s_phi_jes%sDown" % (self.RawmetBranchName, jesUncertainty), math.atan2(Rawmet_py_jesDown[jesUncertainty], Rawmet_px_jesDown[jesUncertainty]))
          ...
          self.out.fillBranch("%s_pt_unclustEnUp" % self.PuppimetBranchName, math.sqrt(Puppimet_px_unclEnUp**2 + Puppimet_py_unclEnUp**2))
          self.out.fillBranch("%s_phi_unclustEnUp" % self.PuppimetBranchName, math.atan2(Puppimet_py_unclEnUp, Puppimet_px_unclEnUp))
          self.out.fillBranch("%s_pt_unclustEnDown" % self.PuppimetBranchName, math.sqrt(Puppimet_px_unclEnDown**2 + Puppimet_py_unclEnDown**2))
          self.out.fillBranch("%s_phi_unclustEnDown" % self.PuppimetBranchName, math.atan2(Puppimet_py_unclEnDown, Puppimet_px_unclEnDown))

          self.out.fillBranch("%s_pt_unclustEnUp" % self.RawmetBranchName, math.sqrt(Rawmet_px_unclEnUp**2 + Rawmet_py_unclEnUp**2))
          self.out.fillBranch("%s_phi_unclustEnUp" % self.RawmetBranchName, math.atan2(Rawmet_py_unclEnUp, Rawmet_px_unclEnUp))
          self.out.fillBranch("%s_pt_unclustEnDown" % self.RawmetBranchName, math.sqrt(Rawmet_px_unclEnDown**2 + Rawmet_py_unclEnDown**2))
          self.out.fillBranch("%s_phi_unclustEnDown" % self.RawmetBranchName, math.atan2(Rawmet_py_unclEnDown, Rawmet_px_unclEnDown))        
