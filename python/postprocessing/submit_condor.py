...
def sub_writer(sample, n, files, folder):
    ...
    f.write("transfer_output_remaps  = \""+ sample.label + "_part" + str(n) + ".root=/eos/user/"+str(os.environ.get('USER')[0])+"/"+str(os.environ.get('USER'))+"/Wprime/nosynch/" + folder +"/"+ sample.label +"/"+ sample.label + "_part" + str(n) + ".root\"\n")
...
for sample in samples:
    ...
    if('Data' in sample.label):
        isMC = False
    if not os.path.exists("/eos/user/"+str(os.environ.get('USER')[0]) + "/"+str(os.environ.get('USER'))+"/Wprime/nosynch/" + folder + "/" + sample.label):
        os.makedirs("/eos/user/"+str(os.environ.get('USER')[0]) + "/"+str(os.environ.get('USER'))+"/Wprime/nosynch/" + folder + "/" + sample.label)
